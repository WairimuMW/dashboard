...
st.set_page_config(layout="wide", initial_sidebar_state="expanded" ) # page configuration
import pandas as pd
import numpy as np
...
st.sidebar.header("Amazure Online Shop") # heading

data = st.sidebar.file_uploader("Upload Dataset", type=['csv', 'txt', 'xlsx']) # helps you upload files
# logic to work on data after upload
if data is not None:
        df = pd.read_csv(data)
...
else: # do this if no file is uploaded
    df = pd.read_csv(url, encoding="ISO-8859-1", low_memory=False)
    df["Revenue"] = df["UnitPrice"] * df["Quantity"]
    df["InvoiceDate"] = pd.to_datetime(df["InvoiceDate"])
    df["InvoiceMonth"] = pd.DatetimeIndex(df["InvoiceDate"]).month
...
menu = ['Business Snapshot','Analysis','About'] # define the menu options
...
# logic to be executed when each option is selected
...
if selection == 'Business Snapshot':
    ...
    col1, col2 = st.columns(2) # define 2 columns for 2 graphs
    ...
    with col1:
        ...
        plt.xlabel("Month")
        plt.ylabel("Revenue")
        st.pyplot(plt) # used to call matplotlib or seaborn graphs
    ...
    col3, col4 = st.columns(2) # 2 more columns for 2 more graphs
    ...
    with col4:
        ...
        df_revenue_avg = df.groupby(["InvoiceMonth"])["Revenue"].mean().reset_index()
        plt.figure(figsize=(15, 10))
